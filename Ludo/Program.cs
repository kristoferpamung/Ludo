...
using LudoGameEnums;
using LudoGameInterfaces;
List<IPlayer> players = [new Player("Windah", ColorState.RED), new Player("Zeta", ColorState.GREEN), new Player("Kobo", ColorState.YELLOW), new Player("Rizad", ColorState.BLUE)];

LudoModel lm = new LudoModel(players, new Board(), new Dice(), new GameController());
lm.board.Grid[6, 1].Pieces.Add(lm.players[0].PlayerPieces[0]);
lm.players[0].PlayerPieces[0].Position.Y = 6;
lm.players[0].PlayerPieces[0].Position.X = 1;
lm.gameController.AddPlayerPieceIntoBoard(lm.board, lm.players);
lm.gameController.DisplayBoard(lm.board, lm.players);
